...
def main():
    ...
    # model
    # -----
    logging.info("using random forests & 3-fold cv")
    param_grid = {
            "n_estimators": [30]
    }
    ...
    model = RandomForestRegressor(random_state=250525)
    ...
    util.print_tree(model, df)
    #util.visualise_tree(model, df, out_dir/f"{mn}.tree")
    ...
    util.plot_correlation(reference=ys_train, predicted=ys_pred_train, out_fn=out_fn)
