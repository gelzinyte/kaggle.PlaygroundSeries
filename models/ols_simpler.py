...
def main():
    ...
    if np.min(ys_pred_train) < 0:
        logging.error
        (
            f"min y pred train: {np.min(ys_pred_train)}, can't log for error metric"
        )
...
if __name__ == "__main__":
    logging.info(
        "fitting directly to dummied scaled shifted features gives negative predictions"
    )
    logging.info("ofc scaling or not scaling doesn't make a difference here")
    main()
